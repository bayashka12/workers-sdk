	...
	for (const page of testCases) {
		const { text, url } = await retry(
			...
			async () => {
				...
				const temp = { text: await r.text(), status: r.status, url: r.url };
				return temp;
			}
		);
		if (page.content) {
		...
		if (page.redirect) {
			expect(url).toEqual(new URL(page.redirect, deployedUrl).toString());
		} else {
			expect(url).toEqual(new URL(page.path, deployedUrl).toString());
		}
	}
};
...
describe("Workers + Assets deployment", { timeout: TIMEOUT }, () => {
	...
	it("deploys a Workers + Assets project with assets only", async () => {
		...
		const testCases: AssetTestCase[] = [
			...
			// TODO - uncomment after new AW deployment is done
			// {
			// 	path: "/[boop]",
	...
	it("should delete Worker + Assets", async () => {
